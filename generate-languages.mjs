...
async function generateLanguagesFile() {
  console.log(
    "🚀 Starting language data generation (with proper binary file detection)..."
  );
  const languagesData = [];
...
`;
    ...
    console.log(
      `✅ Success! Wrote data for ${languagesData.length} languages to ${outputFilePath}`
    );
    if (skippedCount > 0) {
      console.log(`ℹ️ Skipped ${skippedCount} binary files.`);
    }
  } catch (error) {
